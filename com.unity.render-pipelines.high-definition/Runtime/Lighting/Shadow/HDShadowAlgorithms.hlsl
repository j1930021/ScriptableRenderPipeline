...
{
    float4 posCS = EvalShadow_WorldToShadow(sd, positionWS, perspProj);
    float2 posNDC = perspProj ? (posCS.xy / posCS.w) : posCS.xy;
    // calc TCs
...
float EvalShadow_ReceiverBiasWeight(HDShadowData sd, float2 atlasSizeRcp, float2 offset, float4 viewBias, float edgeTolerance, int flags, Texture2D tex, SamplerComparisonState samp, float3 positionWS, float3 normalWS, float3 L, float L_dist, bool perspProj)                              { return 1.0; }
float EvalShadow_ReceiverBiasWeight (Texture2D tex, SamplerState samp, float3 positionWS, float3 normalWS, float3 L, float L_dist, bool perspProj)                                        { return 1.0; }
